...
export default {
  ...
  computed: {
    ...
    filteredTickers() {
      const items = this.tickers.filter(item => item.name.includes(this.filter));;
      return items;
    },
  ...
  methods: {
    async setAllCoins() {
      const data = await this.getData('https://min-api.cryptocompare.com/data/all/coinlist?summary=true')
      this.tickerList = [];
    ...
    filteredTickerList() {
      ...
      for (let i = 0; i < this.tickerValues.length; i++) {
        if ((this.tickerValues[i].Symbol && this.tickerValues[i].Symbol.toLowerCase().includes(this.ticker.toLowerCase())) || this.ticker == undefined) {
    ...
    async getTickerPrice(tickerName) {
      const data = await this.data(`https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&apo_key=${env.api_key}`);
    ...
    subscribeToUpdate(tickerName) {
      this.getTickerPrice(tickerName)
      setInterval(async () => {
        this.getTickerPrice(tickerName)
      }, 12000);
    },
    ...
    add(tickerName) {
      const currentTicker = {name: tickerName || this.ticker, price: "-"};
      if (this.tickers.findIndex((item) => item.name.toLowerCase() === currentTicker.name.toLowerCase()) == -1) {
    ...
    deleteItem(deleteItem) {
      this.tickers = this.tickers.filter((item) => deleteItem != item);
      if (this.selectedTicker === deleteItem) {
